...
// ----------------------------------------------------------------------
// UTILITAIRES DB (copiés du petit serveur)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// INITIALISATION DE LA BASE DE DONNÉES (une seule passe, robuste)
// ----------------------------------------------------------------------
async function initializeDatabase() {
  try {
    ...
    `);
    // Ajout de la colonne agent_id_int pour compatibilité (optionnelle)
    ...
    console.log('✅ Toutes les tables sont créées / vérifiées');
    ...
    for (const d of defaultDraws) {
      await pool.query(`
        INSERT INTO draws (id, name, time, active, blocked)
    ...
    // Création des comptes par défaut (uniquement si aucun utilisateur)
    ...
    if (parseInt(supCount.rows[0].count) === 0) {
      ...
      await pool.query(`
        ...
        VALUES ($1, $2, $3)
      `, ['Superviseur Principal', 'super@lotato.com', hashed]);
    }
    ...
    if (parseInt(agentCount.rows[0].count) === 0) {
      ...
      await pool.query(`
        INSERT INTO agents (name, email, password, supervisor_id, location)
        VALUES ($1, $2, $3, $4, $5)
      `, ['Agent Test', 'agent@lotato.com', hashed, supId, 'Port-au-Prince']);
    }
    ...
    if (parseInt(ownerCount.rows[0].count) === 0) {
      ...
      await pool.query(`
        ...
        VALUES ($1, $2, $3)
      `, ['Administrateur', 'admin@lotato.com', hashed]);
    }
    ...
    // On ne relance pas l'erreur pour permettre au serveur de démarrer
    // mais l'administrateur devra corriger le problème
  }
}
...
const authenticateToken = (req, res, next) => {
  // Routes publiques (chemin complet avec /api)
  const publicRoutes = [
    '/api/health',
    '/api/auth/login',
    '/api/auth/refresh',
...
// ----------------------------------------------------------------------
// ROUTES TICKETS (inchangées, robustes)
// ----------------------------------------------------------------------
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO agents (name, email, phone, password, supervisor_id, location, commission)
       ...
       RETURNING id, name, email, phone, location, commission, active, created_at`,
...
app.put('/api/supervisor/agents/:agentId', requireRole('supervisor', 'owner'), async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE agents SET name = $1, email = $2, phone = $3, location = $4,
         commission = $5, active = $6, updated_at = NOW()
       WHERE id = $7`,
  ...
  try {
    ...
    if (agentIds.length > 0) {
      ...
      const ticketStats = await pool.query(`
        ...
        FROM tickets
        WHERE agent_id::integer IN (${placeholders}) AND DATE(date) = CURRENT_DATE
      `, agentIds);
