...
async function createTablesIfNotExist() {
  try {
    ...
    // Table des superviseurs (inclut les propriétaires)
    await pool.query(`
    ...
    // Table des numéros bloqués globalement
    await pool.query(`
    ...
    // Table des numéros bloqués par tirage
    await pool.query(`
    ...
    // Table des limites de mise par numéro et tirage
    await pool.query(`
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS lottery_config (
        id SERIAL PRIMARY KEY,
        name VARCHAR(255) DEFAULT 'LOTATO PRO',
        slogan TEXT,
        logo TEXT,
        multipliers JSONB,
        game_limits JSONB,
    ...
    // Table des messages propriétaires (pour superadmin)
    await pool.query(`
...
// Initialisation (tables + données par défaut)
async function initializeDatabase() {
  try {
    console.log('🔄 Initialisation de la base de données...');
    await createTablesIfNotExist();

    // Ajout des colonnes manquantes (pour les tables déjà existantes)
    ...
    // Insérer une ligne par défaut dans lottery_config si elle est vide
    const configCount = await pool.query('SELECT COUNT(*) FROM lottery_config');
    if (parseInt(configCount.rows[0].count) === 0) {
      await pool.query(`
        INSERT INTO lottery_config (name, slogan, multipliers, game_limits)
        VALUES ('LOTATO PRO', '', '{"lot1":60,"lot2":20,"lot3":10,"lotto3":500,"lotto4":5000,"lotto5":25000,"mariage":500}', '{"lotto3":0,"lotto4":0,"lotto5":0}')
      `);
      console.log('✅ Configuration par défaut insérée');
    }

    // Insérer un superadmin par défaut (admin@lotato.com / admin123) s'il n'existe pas
    const superadminCount = await pool.query('SELECT COUNT(*) FROM superadmins');
    if (parseInt(superadminCount.rows[0].count) === 0) {
      const hashedPassword = await bcrypt.hash('admin123', 10);
      await pool.query(`
        INSERT INTO superadmins (name, email, password)
        VALUES ('Super Admin', 'admin@lotato.com', $1)
      `, [hashedPassword]);
      console.log('✅ Superadmin par défaut créé (admin@lotato.com / admin123)');
    }

    // Créer quelques tirages par défaut si aucun n'existe
...
app.post('/api/tickets/save', async (req, res) => {
  try {
    ...
    // Vérification des limites par type de jeu (sécurisée si config absente)
...
app.delete('/api/tickets/:ticketId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!['supervisor', 'owner', 'agent'].includes(user.role)) return res.status(403).json({ error: 'Accès interdit' });
    ...
    if (user.role === 'agent') {
      ...
      const agentCheck = await pool.query('SELECT id FROM agents WHERE id = $1 AND supervisor_id = $2', [ticket.agent_id, user.id]);
      if (agentCheck.rows.length === 0) return res.status(403).json({ error: 'Ce ticket n\'est pas sous votre supervision' });
    }

    await pool.query('DELETE FROM tickets WHERE id = $1', [id]);
    await pool.query('INSERT INTO activity_log (user_id, user_role, action, ip_address) VALUES ($1, $2, $3, $4)', [user.id, user.role, 'delete_ticket', req.ip]);
    res.json({ success: true });
  } catch (error) {
...
app.get('/api/winners', async (req, res) => {
  try {
    ...
    const params = [];
    if (agentId) { params.push(agentId); query += ` AND agent_id = $${params.length}`; }
    if (req.user.role === 'agent') { params.push(req.user.id); query += ` AND agent_id = $${params.length}`; }
...
app.get('/api/winners/results', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, draw_id, name, results, lotto3, published_at FROM draw_results ORDER BY published_at DESC LIMIT 10');
    const results = result.rows.map(r => ({ ...r, numbers: typeof r.results === 'string' ? JSON.parse(r.results) : r.results, lotto3: r.lotto3 }));
    res.json({ results });
  } catch (error) {
...
app.post('/api/winners/pay/:ticketId', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'agent') { query += ' AND agent_id = $2'; params.push(req.user.id); }
    const result = await pool.query(query, params);
    if (result.rowCount === 0) return res.status(404).json({ error: 'Ticket non trouvé ou non autorisé' });
    res.json({ success: true });
  } catch (error) {
...
app.get('/api/reports', async (req, res) => {
  try {
    ...
    if (!agentId && req.user.role === 'agent') agentId = req.user.id;
    if (!agentId) return res.status(400).json({ error: 'Agent ID requis' });
    const todayStats = await pool.query(
      `SELECT COUNT(*) as total_tickets, COALESCE(SUM(total_amount), 0) as total_bets, COALESCE(SUM(win_amount), 0) as total_wins,
              COALESCE(SUM(total_amount) - SUM(win_amount), 0) as total_loss, COALESCE(SUM(win_amount) - SUM(total_amount), 0) as balance
...
app.get('/api/reports/draw', async (req, res) => {
  try {
    ...
    const stats = await pool.query(
      `SELECT COUNT(*) as total_tickets, COALESCE(SUM(total_amount), 0) as total_bets, COALESCE(SUM(win_amount), 0) as total_wins,
              COALESCE(SUM(total_amount) - SUM(win_amount), 0) as total_loss, COALESCE(SUM(win_amount) - SUM(total_amount), 0) as balance
       FROM tickets WHERE agent_id = $1 AND draw_id = $2 AND DATE(date) = CURRENT_DATE`,
      [agentId, drawId]
    );
...
supervisorRouter.get('/reports/overall', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT COUNT(DISTINCT t.id) as total_tickets, COALESCE(SUM(t.total_amount), 0) as total_bets,
              COALESCE(SUM(t.win_amount), 0) as total_wins, COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as balance
       FROM tickets t JOIN agents a ON t.agent_id = a.id
...
supervisorRouter.post('/block-agent/:agentId', async (req, res) => {
  try {
    ...
    if (check.rows.length === 0) return res.status(404).json({ error: 'Agent non trouvé ou non autorisé' });
...
supervisorRouter.get('/tickets', async (req, res) => {
  try {
    ...
    let paramIndex = 2;
    if (agentId && agentId !== 'all') { conditions.push(`t.agent_id = $${paramIndex++}`); params.push(agentId); }
    ...
    const dataQuery = `SELECT t.* FROM tickets t JOIN agents a ON t.agent_id = a.id ${whereClause} ORDER BY t.date DESC LIMIT $${paramIndex++} OFFSET $${paramIndex++}`;
...
ownerRouter.get('/dashboard', async (req, res) => {
  try {
    const connectedSupervisors = await pool.query(`SELECT id, name, email FROM supervisors WHERE active = true LIMIT 5`);
    const connectedAgents = await pool.query(`SELECT id, name, email FROM agents WHERE active = true LIMIT 5`);
    const salesToday = await pool.query(`SELECT COALESCE(SUM(total_amount), 0) as total FROM tickets WHERE DATE(date) = CURRENT_DATE`);
...
ownerRouter.get('/supervisors', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, email, phone, active as blocked FROM supervisors ORDER BY name');
...
ownerRouter.get('/agents', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT a.id, a.name, a.email, a.phone, a.active as blocked, s.name as supervisor_name, a.supervisor_id
       FROM agents a LEFT JOIN supervisors s ON a.supervisor_id = s.id ORDER BY a.name`
    );
    res.json(result.rows);
  } catch (error) {
...
ownerRouter.post('/create-user', async (req, res) => {
  try {
    ...
    if (role === 'supervisor') {
      await pool.query(`INSERT INTO supervisors (name, email, password, phone, active) VALUES ($1, $2, $3, $4, true) RETURNING id`, [name, username, hashedPassword, cin || '']);
    } else if (role === 'agent') {
      await pool.query(`INSERT INTO agents (name, email, password, phone, supervisor_id, location, active) VALUES ($1, $2, $3, $4, $5, $6, true) RETURNING id`, [name, username, hashedPassword, cin || '', supervisorId || null, zone || '']);
...
ownerRouter.post('/publish-results', async (req, res) => {
  try {
    ...
    if (!drawId || !numbers || !Array.isArray(numbers) || numbers.length !== 3) return res.status(400).json({ error: 'Données invalides' });
    ...
    await pool.query(`INSERT INTO draw_results (draw_id, name, results, lotto3, draw_time, published_at) VALUES ($1, $2, $3, $4, NOW(), NOW())`, [drawId, draw.rows[0].name, JSON.stringify(numbers), lotto3]);
    await pool.query('UPDATE draws SET last_draw = NOW() WHERE id = $1', [drawId]);
    const lot1 = numbers[0]; const lot2 = numbers[1]; const lot3 = numbers[2];
      ...
      if (Array.isArray(bets)) {
        for (const bet of bets) {
          ...
          if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) {
            ...
            if (cleanNumber.length === 3 && cleanNumber === lotto3) gain = amount * 500;
            ...
            if (cleanNumber.length === 4) {
              ...
              for (let i = 0; i < 3; i++) {
                for (let j = 0; j < 3; j++) {
                  if (i !== j && firstPair === pairs[i] && secondPair === pairs[j]) { win = true; break; }
                }
                if (win) break;
              }
              if (win) gain = (bet.free && bet.freeType === 'special_marriage') ? 1000 : amount * 1000;
            }
...
ownerRouter.post('/number-limit', async (req, res) => {
  try {
    ...
    if (!drawId || !number || !limitAmount) return res.status(400).json({ error: 'drawId, number et limitAmount requis' });
    await pool.query(
  ...
  try {
    const result = await pool.query(
      `SELECT dbn.draw_id, d.name as draw_name, dbn.number
       FROM draw_blocked_numbers dbn JOIN draws d ON dbn.draw_id = d.id
  ...
  try {
    const result = await pool.query(
      `SELECT dnl.draw_id, d.name as draw_name, dnl.number, dnl.limit_amount
       FROM draw_number_limits dnl JOIN draws d ON dnl.draw_id = d.id
...
ownerRouter.get('/reports', async (req, res) => {
  try {
    ...
    let conditions = [], params = [], paramIndex = 1;
    if (agentId && agentId !== 'all') { conditions.push(`t.agent_id = $${paramIndex++}`); params.push(agentId); }
    else if (supervisorId && supervisorId !== 'all') { conditions.push(`a.supervisor_id = $${paramIndex++}`); params.push(supervisorId); }
    if (drawId && drawId !== 'all') { conditions.push(`t.draw_id = $${paramIndex++}`); params.push(drawId); }
    let dateCondition = '';
    if (period === 'today') dateCondition = 'DATE(t.date) = CURRENT_DATE';
    else if (period === 'yesterday') dateCondition = 'DATE(t.date) = CURRENT_DATE - INTERVAL \'1 day\'';
    ...
    const summaryQuery = `
      SELECT COUNT(DISTINCT t.id) as total_tickets, COALESCE(SUM(t.total_amount), 0) as total_bets,
             COALESCE(SUM(t.win_amount), 0) as total_wins, COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as net_result,
      ...
      FROM tickets t LEFT JOIN agents a ON t.agent_id = a.id ${whereClause}
    `;
    const summary = await pool.query(summaryQuery, params);
    let detailQuery = '';
    if (drawId && drawId !== 'all') {
      detailQuery = `SELECT a.name as agent_name, a.id as agent_id, COUNT(t.id) as tickets,
                            COALESCE(SUM(t.total_amount), 0) as bets, COALESCE(SUM(t.win_amount), 0) as wins,
                            COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as result
                     FROM tickets t JOIN agents a ON t.agent_id = a.id ${whereClause}
                     GROUP BY a.id, a.name ORDER BY result DESC`;
    } else {
      detailQuery = `SELECT d.name as draw_name, d.id as draw_id, COUNT(t.id) as tickets,
                            COALESCE(SUM(t.total_amount), 0) as bets, COALESCE(SUM(t.win_amount), 0) as wins,
                            COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as result
                     FROM tickets t JOIN draws d ON t.draw_id = d.id ${whereClause}
                     GROUP BY d.id, d.name ORDER BY result DESC`;
    }
...
ownerRouter.post('/settings', upload.single('logo'), async (req, res) => {
  try {
    ...
    const check = await pool.query('SELECT id FROM lottery_config LIMIT 1');
    if (check.rows.length === 0) {
      await pool.query(`INSERT INTO lottery_config (name, slogan, logo, multipliers, game_limits) VALUES ($1, $2, $3, $4, $5)`,
      ...
      if (updates.length > 0) await pool.query(`UPDATE lottery_config SET ${updates.join(', ')}`, values);
    }
    res.json({ success: true });
  } catch (error) {
...
ownerRouter.get('/tickets', async (req, res) => {
  try {
    ...
    const whereClause = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';
    const countQuery = `SELECT COUNT(*) as total FROM tickets t LEFT JOIN agents a ON t.agent_id = a.id ${whereClause}`;
    const countResult = await pool.query(countQuery, params);
    const total = parseInt(countResult.rows[0].total);
    const hasMore = (page * limit + limit) < total;
    const offset = page * limit;
    const dataQuery = `SELECT t.* FROM tickets t LEFT JOIN agents a ON t.agent_id = a.id ${whereClause} ORDER BY t.date DESC LIMIT $${paramIndex++} OFFSET $${paramIndex++}`;
    params.push(limit, offset);
    const dataResult = await pool.query(dataQuery, params);
    const tickets = dataResult.rows.map(t => ({ ...t, bets: typeof t.bets === 'string' ? JSON.parse(t.bets) : t.bets }));
    res.json({ tickets, hasMore, total });
...
ownerRouter.delete('/tickets/:ticketId', async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO activity_log (user_id, user_role, action, details, ip_address) VALUES ($1, $2, $3, $4, $5)', [req.user.id, 'owner', 'delete_ticket', `Ticket ID: ${ticketId}`, req.ip]);
    res.json({ success: true });
  } catch (error) {
...
superadminRouter.get('/owners', async (req, res) => {
  try {
    const result = await pool.query(`SELECT id, name, email, phone, active FROM supervisors WHERE role = 'owner' OR (role IS NULL AND active = true) ORDER BY name`);
    res.json(result.rows);
  } catch (error) {
...
superadminRouter.post('/owners', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    await pool.query(`INSERT INTO supervisors (name, email, password, phone, active, role) VALUES ($1, $2, $3, $4, true, 'owner')`, [name, email, hashedPassword, phone || '']);
    res.json({ success: true });
  } catch (error) {
...
superadminRouter.get('/agents', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, email, phone, active FROM agents ORDER BY name');
    res.json(result.rows);
  } catch (error) {
...
superadminRouter.get('/supervisors', async (req, res) => {
  try {
    const result = await pool.query(`SELECT id, name, email, phone, active FROM supervisors WHERE role != 'owner' OR role IS NULL ORDER BY name`);
    res.json(result.rows);
  } catch (error) {
...
superadminRouter.get('/reports/owners', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT s.id, s.name, COUNT(DISTINCT a.id) as agent_count, COUNT(DISTINCT t.id) as ticket_count,
             COALESCE(SUM(t.total_amount), 0) as total_bets,
             COALESCE(SUM(t.win_amount), 0) as total_wins,
             COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as net_result
      FROM supervisors s
